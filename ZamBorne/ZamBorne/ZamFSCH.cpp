	...
	if (Initialisation())
	{
		...
		while (true)
		{
			...
			if (m_DsaReq_stVal)
			{
				...
				m_change = true;
				m_zl->info(fmt::format("Mise a jour de {}", m_LN));
				m_zl->info(fmt::format("{}  is updated.", m_LN));
			}
...
// Trg
			...
			if(m_StrTm1_setCal_occPer)
			{
				...
				default:
					...
					m_zl->info(fmt::format("Periode {} period not taken into account ", m_StrTm1_setCal_occType));
					return;
					break;
				}
			...
			case 3 :  	//	ready
				...
				{
					if((m_zp->m_temps >= td) && (m_zp->m_temps < tf)) m_status = 4;
				}
				break;
			case 4: // running
				if(!bper)
...
// maj des commandes
			...
			if(m_SchdEntr_stVal <= 0) m_SchdEntr_stVal = 1;
			if(m_SchdEntr_stVal != SchdEntr) Ecrit(m_LN + "SchdEntr", "stVal", m_SchdEntr_stVal);
// Delai
		finboucle:
			this_thread::sleep_for(chrono::seconds(1));
		}   // boucle
