...
//	Update(m_zp->m_nombase, m_sne + "PhyNam.vendor", m_PhyNam_vendor, to_string(m_zp->m_temps));
//	Update(m_zp->m_nombase, m_sne + "NamPlt_vendor", Lits("NamPlt_vendor"), to_string(m_zp->m_temps));
//	Update(m_zp->m_nombase, m_sne + "Proxy_stVal", Lits("Proxy_stVal"), to_string(m_zp->m_temps));
	return true;
} // Initialisation
...
// Delai
	...
	{
		m_zl->info(fmt::format("LPHD: Init impossible {}", m_nom));
	}
	m_zl->info(fmt::format("LPHD: Arret du serveur {}", m_nom));
} // ZamProcess
