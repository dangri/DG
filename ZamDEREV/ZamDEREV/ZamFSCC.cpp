...
// Ecrire la valeur sur la cible
			...
			if (st == 4)
			{
				...
				if (m_ValMV_mag_f != ValMV)
				{
					...
					if ( ! m_CtlEnt_setSrcRef.empty())
					{
						m_zs->Ecrit(m_IED + "LDEV1/DPMC1.WLimSpt2", "mxVal.f", ValMV);
					}
				}
			}
//	 Delai
	...
	{
		m_zl->info(fmt::format("XSWI: Init impossible {}", m_nom));
	}
	m_zl->info(fmt::format("XSWI: Arret du serveur {}", m_nom));
} // ZamProcess
