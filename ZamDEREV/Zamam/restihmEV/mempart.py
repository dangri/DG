...
class MemPart(object):
    ...
    def __init__(self,adresse="",vs=None):
     try:
       ...
       self.vv.reportlog.error(" error,Mempart " +str(ee))
    ...
    def dicwrite(self, nom, val):
        ...
        # il faut convertir val en fonction du type de dic
           if(len(self.vv.dicvars)==0): self.vv.reportlog.error("Mempart dicwrite dicvars est vide");return False
           ...
           self.mapfile.write(valb)
             ...
             ret=False
             self.vv.reportlog.error("  dicwrite nom=" + nom  + " " + str(ee))
    ...
    def dicread(self,nom):
       ...
       # if(self.vv.sem != None): self.vv.sem.acquire() #(timeout=float(2))
        ...
        try:
           ...
           #self.vv.reportlog.info("dicread  nom={0} typ={1} ad={2} nb={3}".format(nom,typ,ad,valb))
           if(typ=="ull"):
